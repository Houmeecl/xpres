...
class EmailService {
     ...
     */
    async sendPaymentConfirmation(email, documentTitle, documentId, paymentAmount, paymentId, userName) {
     ...
     */
    async sendDocumentCopy(email, documentTitle, documentId, pdfBase64, pdfUrl) {
     ...
     */
    async sendDocumentStatusUpdate(email, documentTitle, documentId, status) {
        let statusText = '';
        let subject = '';
        switch (status) {
            case 'pending':
