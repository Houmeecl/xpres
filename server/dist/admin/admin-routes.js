...
adminRouter.use(admin_middleware_1.requireAdmin);
// Dashboard de administración - Estadísticas generales
adminRouter.get('/dashboard', async (req, res) => {
    try {
        // Obtener estadísticas básicas
        const [leadsCount, messagesCount, dialogflowSessionsCount, templatesCount, rulesCount] = await Promise.all([
            db_1.db.select({ count: db_1.db.fn.count() }).from(schema_1.crmLeads),
            db_1.db.select({ count: db_1.db.fn.count() }).from(schema_1.whatsappMessages),
            db_1.db.select({ count: db_1.db.fn.count() }).from(schema_1.dialogflowSessions),
            db_1.db.select({ count: db_1.db.fn.count() }).from(schema_1.messageTemplates),
            db_1.db.select({ count: db_1.db.fn.count() }).from(schema_1.automationRules)
        ]);
        // Obtener distribución de leads por etapa
        const leadsByStage = await db_1.db
            .select({
            stage: schema_1.crmLeads.pipelineStage,
            count: db_1.db.fn.count()
        })
            .from(schema_1.crmLeads)
            .groupBy(schema_1.crmLeads.pipelineStage);
        // Obtener últimos mensajes de WhatsApp
        const recentMessages = await db_1.db
            .select()
...
adminRouter.get('/users', admin_middleware_1.requireSuperAdmin, async (req, res) => {
    try {
...
adminRouter.patch('/users/:id/role', admin_middleware_1.requireSuperAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { role } = req.body;
        // Validar el rol
        ...
        const [updatedUser] = await db_1.db
            ...
            .set({ role })
            .where((0, drizzle_orm_1.eq)(schema_1.users.id, parseInt(id)))
            .returning();
        if (!updatedUser) {
...
adminRouter.get('/message-templates', async (req, res) => {
    try {
        const templates = await db_1.db
            .select()
            .from(schema_1.messageTemplates)
            .orderBy((0, drizzle_orm_1.desc)(schema_1.messageTemplates.updatedAt));
        res.json(templates);
    }
    catch (error) {
...
adminRouter.post('/message-templates', async (req, res) => {
    try {
        const { name, category, content, variables, isWhatsappTemplate, whatsappTemplateNamespace, whatsappTemplateElementName } = req.body;
        ...
        const [template] = await db_1.db
            .insert(schema_1.messageTemplates)
            .values({
            name,
            category,
            content,
...
adminRouter.patch('/message-templates/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { name, category, content, variables, isWhatsappTemplate, whatsappTemplateNamespace, whatsappTemplateElementName, isActive } = req.body;
        // Actualizar plantilla
        const [template] = await db_1.db
            .update(schema_1.messageTemplates)
            .set({
            name,
            category,
            content,
            ...
            .where((0, drizzle_orm_1.eq)(schema_1.messageTemplates.id, parseInt(id)))
            .returning();
        if (!template) {
...
adminRouter.get('/automation-rules', async (req, res) => {
    try {
        const rules = await db_1.db
            .select()
            .from(schema_1.automationRules)
            .orderBy((0, drizzle_orm_1.desc)(schema_1.automationRules.updatedAt));
        res.json(rules);
    }
    catch (error) {
...
adminRouter.post('/automation-rules', async (req, res) => {
    try {
        const { name, description, triggerType, triggerEvent, triggerSchedule, triggerCondition, actionType, actionConfig, isActive } = req.body;
        // Validar datos obligatorios
        ...
        const [rule] = await db_1.db
            .insert(schema_1.automationRules)
            .values({
            name,
            description,
            triggerType,
...
adminRouter.patch('/automation-rules/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, triggerType, triggerEvent, triggerSchedule, triggerCondition, actionType, actionConfig, isActive } = req.body;
        // Actualizar regla
        const [rule] = await db_1.db
            .update(schema_1.automationRules)
            .set({
            name,
            description,
            triggerType,
            ...
            .where((0, drizzle_orm_1.eq)(schema_1.automationRules.id, parseInt(id)))
            .returning();
        if (!rule) {
...
adminRouter.post('/initialize-admin', admin_middleware_1.requireSuperAdmin, async (req, res) => {
    try {
        await (0, seed_admin_1.createSuperAdmin)();
...
adminRouter.get('/master-dashboard', async (req, res) => {
    try {
        // Obtener estadísticas de documentos
        const [totalDocumentsResult, documentsLastMonthResult] = await Promise.all([
            db_1.db.select({ count: db_1.db.fn.count() }).from(schema_1.documents),
            db_1.db.select({ count: db_1.db.fn.count() }).from(schema_1.documents)
                .where((0, drizzle_orm_1.gte)(schema_1.documents.createdAt, (0, date_fns_1.subMonths)(new Date(), 1)))
        ]);
        ...
        const [totalUsersResult, newUsersLastMonthResult] = await Promise.all([
            db_1.db.select({ count: db_1.db.fn.count() }).from(schema_1.users),
            db_1.db.select({ count: db_1.db.fn.count() }).from(schema_1.users)
                .where((0, drizzle_orm_1.gte)(schema_1.users.createdAt, (0, date_fns_1.subMonths)(new Date(), 1)))
        ]);
        ...
        const documentsByStatusQuery = await db_1.db
            .select({
        ...
        documentsByStatusQuery.forEach(item => {
            switch (item.status) {
                case 'pendiente':
                    documentsByStatus.pending = Number(item.count);
                    break;
                case 'firmado':
                    documentsByStatus.signed = Number(item.count);
                    break;
                case 'certificado':
                    documentsByStatus.certified = Number(item.count);
                    break;
                case 'anulado':
                    documentsByStatus.canceled = Number(item.count);
                    break;
            }
        });
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const dayName = (0, date_fns_1.format)(day, 'EEE').substring(0, 3);
            revenueChart.push({
                name: dayName,
        ...
        const pendingDocuments = await db_1.db
            ...
            .from(schema_1.documents)
            .where((0, drizzle_orm_1.eq)(schema_1.documents.status, 'pendiente'))
...
adminRouter.get('/documents', async (req, res) => {
    try {
        ...
        let query = db_1.db.select({
            id: schema_1.documents.id,
            title: schema_1.documents.title,
            fileName: schema_1.documents.filePath,
            status: schema_1.documents.status,
            createdAt: schema_1.documents.createdAt,
            updatedAt: schema_1.documents.updatedAt,
            userId: schema_1.documents.userId,
            certifierId: schema_1.documents.certifierId
        })
            .from(schema_1.documents)
            .orderBy((0, drizzle_orm_1.desc)(schema_1.documents.createdAt));
        ...
        if (date) {
            ...
            switch (date) {
                case 'today':
                    dateFilter = (0, drizzle_orm_1.and)((0, drizzle_orm_1.gte)(schema_1.documents.createdAt, (0, date_fns_1.startOfDay)(today)), (0, drizzle_orm_1.lte)(schema_1.documents.createdAt, (0, date_fns_1.endOfDay)(today)));
                    break;
                case 'yesterday':
                    const yesterday = (0, date_fns_1.subDays)(today, 1);
                    dateFilter = (0, drizzle_orm_1.and)((0, drizzle_orm_1.gte)(schema_1.documents.createdAt, (0, date_fns_1.startOfDay)(yesterday)), (0, drizzle_orm_1.lte)(schema_1.documents.createdAt, (0, date_fns_1.endOfDay)(yesterday)));
                    break;
                case 'week':
                    dateFilter = (0, drizzle_orm_1.gte)(schema_1.documents.createdAt, (0, date_fns_1.subDays)(today, 7));
                    break;
                case 'month':
                    dateFilter = (0, drizzle_orm_1.gte)(schema_1.documents.createdAt, (0, date_fns_1.subMonths)(today, 1));
                    break;
                case 'year':
                    dateFilter = (0, drizzle_orm_1.gte)(schema_1.documents.createdAt, (0, date_fns_1.subMonths)(today, 12));
                    break;
            }
            if (dateFilter) {
        ...
        if (type) {
            conditions.push((0, drizzle_orm_1.eq)(schema_1.documents.documentType, type));
        }
        if (search) {
            conditions.push((0, drizzle_orm_1.like)(schema_1.documents.title, `%${search}%`));
        }
        ...
        const usersList = userIds.length > 0
            ? await db_1.db.select().from(schema_1.users).where(userIds.length === 1
                ? (0, drizzle_orm_1.eq)(schema_1.users.id, userIds[0])
                : inArray(schema_1.users.id, userIds))
            : [];
        // Obtener información de certificadores
        const certifiersList = certifierIds.length > 0
            ? await db_1.db.select().from(schema_1.users).where(certifierIds.length === 1
                ? (0, drizzle_orm_1.eq)(schema_1.users.id, certifierIds[0])
                : inArray(schema_1.users.id, certifierIds))
            : [];
...
adminRouter.get('/certifiers', async (req, res) => {
    try {
        const { status, region } = req.query;
        // Obtener certificadores (usuarios con rol 'certifier')
        let query = db_1.db.select().from(schema_1.users).where((0, drizzle_orm_1.eq)(schema_1.users.role, 'certifier'));
        // Aplicar filtros si existen
        if (status && status !== 'all') {
            query = query.where((0, drizzle_orm_1.eq)(schema_1.users.isActive, status === 'active'));
        }
...
adminRouter.get('/ai-analysis', async (req, res) => {
    try {
        ...
        const operationalData = await getOperationalData(type);
        ...
        try {
            ...
            analysisData = await openaiService.generateStrategicAnalysis(type, operationalData, { model: "gpt-4o" });
            ...
            for (let i = 0; i < 30; i++) {
                const date = (0, date_fns_1.format)(addDays(today, i), 'dd/MM');
            ...
            analysisData = getFallbackAnalysisData(type);
        }
...
adminRouter.post('/ai-analysis/generate', async (req, res) => {
    try {
        // Importar el servicio de OpenAI
        const { openaiService } = await Promise.resolve().then(() => __importStar(require('../services/openai-service')));
        ...
        const analysisData = await openaiService.generateStrategicAnalysis(analysisType, operationalData, {
            model,
            depth,
...
async function getOperationalData(analysisType) {
    ...
    const baseData = {
        ...
        period: {
            start: (0, date_fns_1.format)(lastMonth, 'yyyy-MM-dd'),
...
function getFallbackAnalysisData(type) {
    return {
        analysisTitle: 'Análisis ' + (type === 'business' ? 'de Negocio' :
            type === 'market' ? 'de Mercado' :
                type === 'expansion' ? 'de Expansión' :
                    type === 'pricing' ? 'de Precios' :
                        type === 'marketing' ? 'de Marketing' : 'General'),
        generatedAt: (0, date_fns_1.format)(new Date(), 'dd/MM/yyyy HH:mm'),
        model: 'gpt-4o',
...
async function generateMarketingEmails() {
    try {
        ...
        const abandonmentEmail = await openaiService.generateMarketingEmail("Recuperar clientes que abandonaron el proceso de certificación", "Usuarios que iniciaron pero no completaron el proceso de certificación", {
        ...
        const promotionEmail = await openaiService.generateMarketingEmail("Promover paquete empresarial de certificaciones", "Empresas con necesidades frecuentes de certificación documental", {
