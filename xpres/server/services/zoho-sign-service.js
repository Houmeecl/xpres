...
exports.getSigningRequestStatus = getSigningRequestStatus;
exports.downloadSignedDocument = downloadSignedDocument;
exports.sendReminder = sendReminder;
exports.cancelSigningRequest = cancelSigningRequest;
exports.verifyZohoAuthentication = verifyZohoAuthentication;
...
async function getAccessToken() {
    ...
    try {
        ...
        const response = await axios_1.default.post(`${ZOHO_ACCOUNTS_URL}/oauth/v2/token`, null, {
            params: {
 ...
 */
async function createSigningRequest(signRequest) {
    try {
        const token = await getAccessToken();
        const formData = new form_data_1.default();
        formData.append('document_name', signRequest.document_name);
        // Adjuntar el documento
        const documentData = await fs_1.promises.readFile(signRequest.document_path);
        formData.append('file', documentData, path_1.default.basename(signRequest.document_path));
        ...
        const response = await axios_1.default.post(`${ZOHO_SIGN_API_URL}/requests`, formData, {
            headers: {
                ...formData.getHeaders(),
...
async function getSigningRequestStatus(requestId) {
    try {
        const token = await getAccessToken();
        const response = await axios_1.default.get(`${ZOHO_SIGN_API_URL}/requests/${requestId}`, {
            headers: {
                'Authorization': `Zoho-oauthtoken ${token}`
            }
        });
 ...
 */
async function downloadSignedDocument(requestId, documentId, outputPath) {
    try {
        const token = await getAccessToken();
        const response = await axios_1.default.get(`${ZOHO_SIGN_API_URL}/requests/${requestId}/documents/${documentId}/pdf`, {
            headers: {
                'Authorization': `Zoho-oauthtoken ${token}`
            },
            responseType: 'arraybuffer'
        });
        await fs_1.promises.writeFile(outputPath, response.data);
        return outputPath;
    }
    catch (error) {
...
async function sendReminder(requestId) {
    try {
        const token = await getAccessToken();
        const response = await axios_1.default.post(`${ZOHO_SIGN_API_URL}/requests/${requestId}/reminder`, {}, {
            headers: {
                'Authorization': `Zoho-oauthtoken ${token}`,
 ...
 * @returns Respuesta de la API
 */
async function cancelSigningRequest(requestId) {
    try {
        const token = await getAccessToken();
        const response = await axios_1.default.delete(`${ZOHO_SIGN_API_URL}/requests/${requestId}`, {
            headers: {
                'Authorization': `Zoho-oauthtoken ${token}`
            }
        });
 ...
 */
async function verifyZohoAuthentication() {
    try {
