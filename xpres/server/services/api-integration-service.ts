...
let integrations: Record<string, ApiIntegration> = {
  crm: {
    id: 'crm',
 ...
 */
export async function getIntegrationsStatus(): Promise<Record<string, ApiStatus>> {
 ...
 */
export async function testIntegrationConnection(apiId: string): Promise<ApiStatus> {
  if (!integrations[apiId]) {
    throw new Error(`Integración no encontrada: ${apiId}`);
  }
 ...
 */
export async function getIntegrationConfig(apiId: string): Promise<ApiIntegration> {
  if (!integrations[apiId]) {
    throw new Error(`Integración no encontrada: ${apiId}`);
  }
  ...
  const safeCredentials: ApiCredentials = {};
